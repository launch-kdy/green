...
const Checkbox = () => {
  ...
  //const [fruits, setFruites] = useState([]); //체크 박스 전체 해제

  //마운트 시 체크하고 싶으면 초기값을 지정해주면 됨
  const [fruits, setFruites] = useState(['apple']); 
  ...
  const result = arr.filter((num) => {return num > 1}); //[2,3]
  ...
  const handleCheckbox = (e) => {
    ...
    if(e.target.checked){
      //체크박스의 value를 추가한다.
      setFruites([...fruits, e.target.value]);  
    }
    //체크 해제 했으면...
    else{
      //체크박스에 저장된 값을 제거한다.
      const result = fruits.filter(data => data !== e.target.value); 
      setFruites(result);
    }
  }
