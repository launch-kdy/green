...
CREATE TABLE FREE_BOARD(
	...
	MEM_ID VARCHAR(20) REFERENCES BOARD_MEMBER (MEM_ID),	#작성자(회원ID) +  REFERENCES 참조할 테이블명 (컬럼명)
	CREATE_DATE DATETIME				#작성일
);

# 이러면 LOCK 걸림 - REFERENCES BOARD_MEMBER (MEM_ID) - 사용해서 사전방지 했기 때문에
...
SELECT TITLE, M.MEM_ID, MEM_NAME
FROM BOARD_MEMBER M INNER JOIN FREE_BOARD F
ON M.MEM_ID = F.MEM_ID
WHERE M.MEM_ID LIKE '%a%';

#2. 2025-01-07일 이후에 작성된 게시글의 
#글번호, 제목, 작성일, 작성자ID, 작성자의 나이를 조회하되, 작성일이 최신인 글부터 조회하시오.
SELECT BOARD_NUM, TITLE, M.MEM_ID, MEM_AGE
FROM BOARD_MEMBER M INNER JOIN FREE_BOARD F
ON M.MEM_ID = F.MEM_ID
WHERE CREATE_DATE >= '2025-01-07'
ORDER BY CREATE_DATE DESC;

# 아주 아주 아주 아주 아주 아주 아주 정말 엄청 중요한 거라고 함.
