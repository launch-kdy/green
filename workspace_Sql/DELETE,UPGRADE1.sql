...
#다음과 같은 요구사항을 충족하는 구매이력 테이블(SALE INFO)을 만들어보자.
#구매정보 테이블의 컬럼은 구매번호, 구매상품명, 구매수량, 상품가격
#구매자명,구매일시(년월일시분초)를 가진다.
#PK는 구매번호로 지정하며, 1부터 1씩 자동 증가되게 한다.
#구매상품명, 구매자명은 NULL이 들어갈 수 없다.
#구매수량 데이터를 입력하지 않으면 기본적으로 1일 들어가도록 설정한다.
#구매일 시는 기본값으로 현재 날짜 및 시간이 들어가도록 한다.
#구매 상품명은 중복 데이터를 가질 수 없다.


# [DROP TABLE 테이블명;] 잘못입력하였을때는, 테이블을 지워주고
# Ctrl Alt F9를 눌러서 테이블을 재실행 해준다.
...
#컬럼 만들기.
	...
	ITEM_NAME VARCHAR(10) UNIQUE NOT NULL, #NULL이 들어갈 수 없다. + UNIQUE
	BUY_CNT INT DEFAULT 1,
	BUY_PRICE INT,
	BUY_USER VARCHAR(10) NOT NULL,#NULL이 들어갈 수 없다.
	BUY_DATE DATETIME DEFAULT SYSDATE()
);

INSERT INTO SALE_INFO (ITEM_NAME, BUY_CNT, BUY_PRICE, BUY_USER) VALUES ();
VALUE ('블라우스', 2, 30000, '윤인사');

SELECT * FROM SALE_INFO;
COMMIT;

#1. 상품가격이 10000원 이상인 상품을 구매한 구매자명, 상품가격, 상품명을 조회
SELECT BUY_USER, BUY_PRICE, ITEM_NAME
FROM sale_info
WHERE ITEM_FRICE >= 10000;

#2. SALE_NUM이 2번 이상인 구매 정보 중에서 구매자명에 '바'가 포함되거나,
#	'이'씨가 구매한 구매정보의 모든 칼럼정보를 조회하시오.
...
#	 구매 가격이 5000~20000인 구매정보의 구매자명, 구매상품
