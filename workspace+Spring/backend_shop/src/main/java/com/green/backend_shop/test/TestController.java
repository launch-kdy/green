...
// 리액트에 결과를 전달할때는 데이터만 전달하는게 아니라 여러정보를 함께 전달해는 것이 좋다
// 리액트에 데이터 뿐만 아니라 추가적인 데이터도 함께 보내기 위해 제공되는 객체
// ResponseEntity 라는 객체
@RestController
@RequestMapping("/test")
public class TestController {
  ...
  public ResponseEntity<?> test4(){
    //리액트로 가져갈 데이터가 없다면 status 적용하고 마지막에 .build() 데이터사용
