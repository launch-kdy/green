...
public class FileUploadUtil {
  ...
  //단일 파일 업로드 기능
  //매개변수로 업로드할 파일에 1개 전달 됨
  public static BookImgDTO fileUpload(MultipartFile img){
    // 1) 파일 업로드 경로 지정
    // (본인 같은 경우는) (경로) workspace+Spring
    String uploadPath = "D:\\01-STUDY\\dev\\workspace\\workspace+Spring\\backend_shop\\src\\main\\resources\\static\\upload\\";
  ...
  //파일 다중 업로드 기능
  //매개변수로 업로드 할 파일이 여러개 [배열] 형태로 전달 됨
  public static List<BookImgDTO> multipleFileUpload(MultipartFile[] imgs){
    ...
    for(MultipartFile img : imgs){
      BookImgDTO imgDTO = fileUpload(img); // 이거 여러번 하면 된다.
      imgDTO.setIsMain("N");
      imgList.add(imgDTO);
    }
    return imgList;
